...
# excerpt: "Korábban már foglalkoztunk az idővonal készítésével. Akkor a `vistime` csomagot használtuk. Most a `timevis`-t. (Ennyit a kreatív névadásról `;-)`.) Mindkettő javascript alapú adatvizualizáció. A Gant diagrammok is tulajdonképpen idővonalon megjelenített folyamatok. A posztban azt is ki akartam próbálni, hogy miként lehet az Rpubs ábrát beágyazni ebbe a "statikus" github.io weboldalba."
lang: hu
---
Egy korábbi [posztban]({% post_url 2019-07-19-timeline %}) már foglalkoztunk az idővonal készítésével. Akkor a `vistime` csomagot használtuk. Most a `timevis`-t. (<span style="font-size: 48px; color: Dodgerblue;"><i class='far fa-flushed'></i></span>Ennyit a kreatív névadásról `;-)`.) Mindkettő javascript alapú adatvizualizáció. A Gant diagrammok is tulajdonképpen idővonalon megjelenített folyamatok. A posztban azt is ki akartam próbálni, hogy miként lehet az Rpubs ábrát beágyazni ebbe a "statikus" github.io weboldalba.  
